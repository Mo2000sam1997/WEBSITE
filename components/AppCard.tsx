...
const getCategoryConfig = (tag: string) => {
  // Match met AutomationsPage kleuren
...
export default function AppCard({ app, index = 0 }: AppCardProps) {
  ...
  return (
    <Link 
      href={`/apps/${app.slug}`}
      className="group bg-bg-secondary rounded-lg overflow-hidden transition-all duration-300 flex flex-col w-full shadow-sm hover:shadow-lg hover:scale-[1.02]"
    >
      {/* Main Image Container - exact zoals AutomationsPage */}
      <div className="relative w-full aspect-[1000/788] overflow-hidden bg-bg-primary">
        ...
        {/* Category Badge - Top Right met glow effect */}
        ...
        {/* Black Gradient Overlay - Bottom (exact zoals AutomationsPage) */}
