...
import { Box, Clock, Sparkles, ArrowRight } from "lucide-react";
import appsData from "@/data/apps.json";

const THREE_D_APPS = appsData.apps.filter(app => app.category === "3D");
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          >
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-6">
              ...
              />
              <span className="text-sm font-medium" style={{ color: CATEGORY_COLOR }}>3D Generation</span>
            </div>

            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-6">
              Create stunning{" "}
              <span style={{ color: CATEGORY_COLOR }}>3D models</span>
              <br />with AI
            </h1>
            
            <p className="text-lg text-text-secondary max-w-2xl mx-auto mb-8">
              Transform text and images into detailed 3D models. Perfect for games, product visualization, and creative projects.
            </p>

            {/* Stats */}
            <div className="flex items-center justify-center gap-8 mb-12">
              <div className="text-center">
                <div className="text-2xl font-bold text-white">{THREE_D_APPS.length}</div>
                <div className="text-sm text-text-tertiary">Tools</div>
              </div>
              <div className="w-px h-8 bg-white/10" />
              <div className="text-center">
                <div className="text-2xl font-bold text-white">GLB</div>
                <div className="text-sm text-text-tertiary">Export</div>
              </div>
              <div className="w-px h-8 bg-white/10" />
              <div className="text-center">
                <div className="text-2xl font-bold text-white">~2min</div>
                <div className="text-sm text-text-tertiary">Generation</div>
              </div>
            </div>
          </motion.div>
        </div>
                ...
                <Link href={app.status === "live" ? `/apps/${app.slug}` : "#"}>
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      >
                        <Box size={24} style={{ color: CATEGORY_COLOR }} />
                      </div>
